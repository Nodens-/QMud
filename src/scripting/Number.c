...
bc_num bc_new_num(length, scale)
int    length, scale;
{
	bc_num temp;
...
void bc_free_num(num) bc_num *num;
{
	if (*num == NULL)
		return;
	(*num)->n_refs--;
...
void bc_init_num(num) bc_num *num;
{
...
static void _bc_rm_leading_zeros(num) bc_num num;
{
...
static int _bc_do_compare(n1, n2, use_sign, ignore_last)
bc_num     n1, n2;
int        use_sign;
int        ignore_last;
{
...
char   bc_is_near_zero(num, scale)
bc_num num;
int    scale;
{
	int   count;
...
static bc_num _bc_do_add(n1, n2, scale_min)
bc_num        n1, n2;
int           scale_min;
{
	bc_num sum;
...
static bc_num _bc_do_sub(n1, n2, scale_min)
bc_num        n1, n2;
int           scale_min;
{
	bc_num diff;
...
void bc_sub(n1, n2, result, scale_min) bc_num n1, n2, *result;
int  scale_min;
{
	bc_num diff = NULL;
	int    cmp_res;
...
void bc_add(n1, n2, result, scale_min) bc_num n1, n2, *result;
int  scale_min;
{
	bc_num sum = NULL;
	int    cmp_res;
...
static bc_num new_sub_num(length, scale, value)
int           length, scale;
char         *value;
{
	bc_num temp;
...
void bc_multiply(n1, n2, prod, scale) bc_num n1, n2, *prod;
int  scale;
{
	bc_num pval;
	int    len1, len2;
...
static void    _one_mult(num, size, digit, result) unsigned char *num;
int            size, digit;
...
int    bc_divide(n1, n2, quot, scale)
bc_num n1, n2, *quot;
int    scale;
{
	bc_num         qval;
...
int    bc_divmod(num1, num2, quot, rem, scale)
...
int    bc_modulo(num1, num2, result, scale)
...
int    bc_raisemod(base, expo, mod, result, scale)
...
void bc_raise(num1, num2, result, scale) bc_num num1, num2, *result;
int  scale;
{
	bc_num temp, power;
	long   exponent;
...
void bc_int2num(num, val) bc_num *num;
int  val;
{
	char  buffer[30];
	char *bptr, *vptr;
...
void  bc_str2num(num, str, scale) bc_num *num;
char *str;
int   scale;
{
	int   digits, strscale;
