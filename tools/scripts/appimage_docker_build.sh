...
QMUD_APPIMAGE_DOCKER_QT_PREFIX=${QMUD_APPIMAGE_DOCKER_QT_PREFIX:-/opt/Qt/latest/gcc_64}
...
"$CMAKE_EXE" -S "$PROJECT_DIR" -G Ninja -B "$BUILD_DIR" \
  ...
  -DQt6TextToSpeech_DIR="$QMUD_APPIMAGE_DOCKER_QT_PREFIX/lib/cmake/Qt6TextToSpeech" \
  -DQMUD_ENABLE_APPIMAGE=ON \
  -DQMUD_APPIMAGE_STRICT_QT_SOURCE=ON \
