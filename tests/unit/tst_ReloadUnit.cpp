	...
	class tst_ReloadUnit : public QObject
	{
		...
		private slots:
			void parseReloadStartupArgumentsRequiresBothValues()
			{
				QString statePath;
				QString token;
				QVERIFY(!parseReloadStartupArguments(
				    {QStringLiteral("QMud"), QStringLiteral("--reload-state=a")}, &statePath, &token));
				QVERIFY(!parseReloadStartupArguments(
				    {QStringLiteral("QMud"), QStringLiteral("--reload-token=b")}, &statePath, &token));
				QVERIFY(parseReloadStartupArguments({QStringLiteral("QMud"),
				                                     QStringLiteral("--reload-state=/tmp/x"),
			...
			void writeAndReadRoundTrip()
			{
				...
				world.sequence                  = 1;
				world.presentationCount         = 3;
				world.activePresentationOrdinal = 2;
				...
				QCOMPARE(parsed.worlds.at(0).presentationCount, 3);
				QCOMPARE(parsed.worlds.at(0).activePresentationOrdinal, 2);
  ...
  "worlds": [
    {
      ...
      "notes": ""
			...
			void loadValidatedAndConsumeRemovesStateOnSuccess()
			{
				...
				QVERIFY(loadValidatedAndConsumeReloadStateSnapshot(path, input, &parsed, &error,
				                                                   &cleanupWarning));
				QVERIFY(error.isEmpty());
