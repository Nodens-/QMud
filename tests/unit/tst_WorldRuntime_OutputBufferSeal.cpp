...
namespace
{
	...
	class tst_WorldRuntime_OutputBufferSeal : public QObject
	{
		...
		private slots:
			...
			void sealedContextEndDefersHiddenReplacementRemovalUntilSnapshotRelease()
			{
				WorldRuntime runtime;
				runtime.addLine(QStringLiteral("history"), WorldRuntime::LineOutput);
				runtime.beginIncomingLineLuaContext(QStringLiteral("hidden"), WorldRuntime::LineOutput, {});
				QVERIFY(runtime.reserveIncomingLineLuaContextInBuffer());
				QVERIFY(runtime.hideBufferedIncomingLineLuaContextForReplacement());
				...
				QCOMPARE(runtime.incomingLineLuaContextAbsoluteNumber(), qint64{0});
				QCOMPARE(runtime.lines().size(), 3);
				QCOMPARE(runtime.luaContextLinesInBufferCount(), 2);
				WorldRuntime::LineEntry logicalSecondLine;
				QVERIFY(runtime.luaContextLineEntry(2, logicalSecondLine));
				QCOMPARE(logicalSecondLine.text, QStringLiteral("following"));
				QVERIFY(!runtime.luaContextLineEntryByAbsoluteNumber(hiddenLineNumber, logicalSecondLine));
				runtime.beginIncomingLineLuaContext(QStringLiteral("next pending"), WorldRuntime::LineOutput,
				                                    {});
				QCOMPARE(runtime.luaContextLinesInBufferCount(), 3);
				QVERIFY(runtime.luaContextLineEntry(3, logicalSecondLine));
				QCOMPARE(logicalSecondLine.text, QStringLiteral("next pending"));
				runtime.setSessionStateOutputBufferSealed(false);
				QCOMPARE(runtime.lines().size(), 2);
				QCOMPARE(runtime.lines().at(0).text, QStringLiteral("history"));
				QCOMPARE(runtime.lines().at(1).text, QStringLiteral("following"));
				QCOMPARE(runtime.luaContextLinesInBufferCount(), 3);
				QVERIFY(runtime.luaContextLineEntry(3, logicalSecondLine));
				QCOMPARE(logicalSecondLine.text, QStringLiteral("next pending"));
				runtime.endIncomingLineLuaContext();
