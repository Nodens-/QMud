	...
	class tst_UpdateCheckUtils : public QObject
	{
		...
		private slots:
			...
			void normalizeSha256DigestAcceptsCanonicalForms()
			{
				...
				QCOMPARE(QMudUpdateCheck::normalizeSha256Digest(QStringLiteral("0x%1").arg(sha.toUpper())),
				         sha);
			}
			...
			void evaluateLatestReleasePayloadIgnoresCiRelease()
			{
				const QByteArray payload =
				    makeLatestReleasePayload(QStringLiteral("v10.08-ci"), QJsonArray());
				const auto result = QMudUpdateCheck::evaluateLatestReleasePayload(
			...
			void evaluateLatestReleasePayloadOffersStableReleaseForEqualCiVersion()
			{
				...
				const auto       result  = QMudUpdateCheck::evaluateLatestReleasePayload(
				    payload, QStringLiteral("10.04-ci"), QString(),
				    QMudUpdateCheck::InstallTarget::LinuxAppImage);
			...
			void evaluateLatestReleasePayloadUpdateAvailableAndSkipReset()
			{
				...
				const QString changelog =
				    QStringLiteral("## What's changed\n\n### Bug Fixes\n\n* Fix release notes rendering by "
				                   "@Nodens- ([#131](https://github.com/Nodens-/QMud/pull/131))\n");
				const QByteArray payload =
				    makeLatestReleasePayload(QStringLiteral("v10.07"), assets, changelog);
				...
				QCOMPARE(result.changelog, changelog);
