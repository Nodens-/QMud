	...
	class tst_WorldSessionStateUtils : public QObject
	{
		...
		private slots:
			...
			void writeOutputOnlyAndHistoryOnly()
			{
				...
				QVERIFY(
				    QMudWorldSessionState::readSessionStateFile(historyOnlyPath, &historyOnlyRead, &error));
				...
				QCOMPARE(customOnlyRead.customMxpElements.at(0).name,
				         customOnly.customMxpElements.at(0).name);
				QCOMPARE(customOnlyRead.mxpSessionState.enabled, customOnly.mxpSessionState.enabled);
				QCOMPARE(customOnlyRead.mxpSessionState.puebloActive,
				         customOnly.mxpSessionState.puebloActive);
				...
				QCOMPARE(customOnlyRead.mxpSessionState.previousMode,
			...
			void excludedHiddenOutputLineIsNotSerialized()
			{
				...
				QMudWorldSessionState::WorldSessionStateData writeData;
				writeData.hasOutputBuffer = true;
				for (qint64 lineNumber = 1; lineNumber <= 3; ++lineNumber)
				{
					WorldRuntime::LineEntry line = makeSampleLine();
					line.lineNumber              = lineNumber;
					line.text                    = QStringLiteral("line-%1").arg(lineNumber);
					if (lineNumber == 2)
				...
				QMudWorldSessionState::WorldSessionStateData readData;
				QVERIFY(QMudWorldSessionState::readSessionStateFile(statePath, &readData, &error));
				QCOMPARE(readData.outputLines.size(), 2);
				QCOMPARE(readData.outputLines.at(0).text, QStringLiteral("line-1"));
				QCOMPARE(readData.outputLines.at(1).text, QStringLiteral("line-3"));
			}
